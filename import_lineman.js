...
const CANADIAN_CITY_COORDS = {
  ...
  'edmonton':{lat:53.5461,lng:-113.4938},'calgary':{lat:51.0447,lng:-114.0719},
  'montreal':{lat:45.5017,lng:-73.5673},'winnipeg':{lat:49.8951,lng:-97.1384},
  'regina':{lat:50.4452,lng:-104.6189},'halifax':{lat:44.6488,lng:-63.5752},
  'fredericton':{lat:45.9636,lng:-66.6431},"st. john's":{lat:47.5615,lng:-52.7126},
  'moncton':{lat:46.0878,lng:-64.7782},'saint john':{lat:45.2733,lng:-66.0633},
};
...
// Read file
const wb = xlsx.readFile(process.env.HOME + '/Downloads/All_ IBEW Outside List.xlsx');
...
locals.sort((a,b) => parseInt(a.name.replace('IBEW Local ','')) - parseInt(b.name.replace('IBEW Local ','')));
...
const startIdx = code.indexOf('const IBEW_LINEMAN_LOCALS = [');
const endIdx = code.indexOf('];', startIdx) + 2;
if (startIdx === -1) { console.error('ERROR: IBEW_LINEMAN_LOCALS not found'); process.exit(1); }
code = code.slice(0, startIdx) + `const IBEW_LINEMAN_LOCALS = [\n${jsEntries}\n];` + code.slice(endIdx);
fs.writeFileSync('src/App.jsx', code);

console.log(`\n✅ Updated IBEW_LINEMAN_LOCALS with ${locals.length} locals`);
console.log('🎉 Done! Now run:');
console.log('   git add src/App.jsx && git commit -m "feat: update IBEW Lineman to 140 locals" && git push\n');
