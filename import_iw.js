...
const CANADIAN_CITY_COORDS = {
  ...
  'windsor':{lat:42.3149,lng:-83.0364},'thunder bay':{lat:48.3809,lng:-89.2477},
  'sudbury':{lat:46.4917,lng:-80.9930},'victoria':{lat:48.4284,lng:-123.3656},
  'vancouver':{lat:49.2827,lng:-123.1207},'edmonton':{lat:53.5461,lng:-113.4938},
  'calgary':{lat:51.0447,lng:-114.0719},'montreal':{lat:45.5017,lng:-73.5673},
  'winnipeg':{lat:49.8951,lng:-97.1384},'regina':{lat:50.4452,lng:-104.619},
  'halifax':{lat:44.6488,lng:-63.5752},'fredericton':{lat:45.9636,lng:-66.6431},
  "st. john's":{lat:47.5615,lng:-52.7126},'moncton':{lat:46.0878,lng:-64.7782},
  'saint john':{lat:45.2733,lng:-66.0633},'saskatoon':{lat:52.1332,lng:-106.6700},
  'cambridge':{lat:43.3616,lng:-80.3144},'st. catharines':{lat:43.1594,lng:-79.2469},
};

const CANADIAN_CITY_TO_PROV = {
  'hamilton':'ON','london':'ON','toronto':'ON','ottawa':'ON','windsor':'ON',
  'thunder bay':'ON','sudbury':'ON','cambridge':'ON','st. catharines':'ON',
  'victoria':'BC','vancouver':'BC','edmonton':'AB','calgary':'AB',
  'montreal':'QC','winnipeg':'MB','regina':'SK','saskatoon':'SK',
  'halifax':'NS','fredericton':'NB','saint john':'NB','moncton':'NB',"st. john's":'NL',
};
...
function parseLocation(cityState, address) {
  ...
  for (const {k, p} of canadianProvs) {
    if (cs.includes(k) || addr.includes(k)) {
      const city = cs.split(',')[0].split('-')[0].trim();
      return { city, state: p, coords: CANADIAN_CITY_COORDS[city] || PROV_COORDS[p] };
    }
  }
  if (cs.includes('canada') || cs.includes(', bc') || cs.includes(', ab') || cs.includes(', on')) {
    const city = cs.split(',')[0].split('-')[0].trim().replace(' canada','').trim();
    const prov = CANADIAN_CITY_TO_PROV[city] || 'ON';
    return { city, state: prov, coords: CANADIAN_CITY_COORDS[city] || PROV_COORDS[prov] };
  }

  for (const [name, code] of Object.entries(STATE_ABBR)) {
    if (cs.includes(name)) {
      // Handle "CITY - STATE" format (e.g. "FREDERICKSBURG - MA")
      const city = cs.split(',')[0].split('-')[0].trim();
  ...
  const dashMatch = cs.match(/^(.+?)\s*-\s*([a-z]{2})$/);
  if (dashMatch) {
    const city = dashMatch[1].trim();
    const state = dashMatch[2].toUpperCase();
    if (STATE_COORDS[state]) return { city, state, coords: STATE_COORDS[state] };
  }

  const parts = cs.split(',');
  let state = null, city = parts[0].split('-')[0].trim();
  if (parts.length >= 2) {
...
// Read file
const wb = xlsx.readFile(process.env.HOME + '/Downloads/All_ Iron Workers List.xlsx');
...
rows.forEach(row => {
  const localNum = row['Local '] || row['Local'];
  if (!localNum) return;
  const num = Math.floor(parseFloat(String(localNum)));
  if (isNaN(num)) return;
  if (seenLocals.has(num)) { console.log('Duplicate:', num); return; }
  seenLocals.add(num);
...
locals.sort((a,b) => parseInt(a.name.replace('IW Local ','')) - parseInt(b.name.replace('IW Local ','')));

const withCoords = locals.filter(l => l.lat).length;
const canadian = locals.filter(l => PROV_COORDS[l.state]).length;
console.log(`Unique: ${locals.length}, With coords: ${withCoords}, Canadian: ${canadian}`);
...
const startIdx = code.indexOf('const IW_LOCALS = [');
const endIdx = code.indexOf('];', startIdx) + 2;
if (startIdx === -1) { console.error('ERROR: IW_LOCALS not found'); process.exit(1); }
code = code.slice(0, startIdx) + `const IW_LOCALS = [\n${jsEntries}\n];` + code.slice(endIdx);
fs.writeFileSync('src/App.jsx', code);

console.log(`\n✅ Updated IW_LOCALS with ${locals.length} locals`);
console.log('🎉 Done! Now run:');
console.log('   git add src/App.jsx && git commit -m "feat: update Ironworkers to 135 locals" && git push\n');
