...
// ============================================================
// STEP 1: Create scripts/generate-og-pages.mjs
// ============================================================
const scriptsDir = 'scripts';
if (!fs.existsSync(scriptsDir)) fs.mkdirSync(scriptsDir);

const ogScript = `import fs from 'node:fs';
...
fs.writeFileSync(path.join(scriptsDir, 'generate-og-pages.mjs'), ogScript);
console.log('✓ Created scripts/generate-og-pages.mjs');

// ============================================================
// STEP 2: Add postbuild to package.json
// ============================================================
const pkgPath = 'package.json';
if (!fs.existsSync(pkgPath)) { console.error('ERROR: package.json not found'); process.exit(1); }

const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf-8'));
if (!pkg.scripts) pkg.scripts = {};

if (pkg.scripts.postbuild === 'node scripts/generate-og-pages.mjs') {
  console.log('✓ postbuild script already configured');
} else {
  pkg.scripts.postbuild = 'node scripts/generate-og-pages.mjs';
  fs.writeFileSync(pkgPath, JSON.stringify(pkg, null, 2) + '\n');
  console.log('✓ Added postbuild to package.json');
}

// ============================================================
// STEP 3: Delete the broken middleware.js
// ============================================================
if (fs.existsSync('middleware.js')) {
  fs.unlinkSync('middleware.js');
  console.log('✓ Removed broken middleware.js');
} else {
  console.log('✓ middleware.js already removed');
}

console.log('\nDone! Now run:');
console.log('   git add scripts/ package.json && git rm middleware.js 2>/dev/null; git add -A && git commit -m "feat: per-page OG tags via build-time HTML generation" && git push\n');
